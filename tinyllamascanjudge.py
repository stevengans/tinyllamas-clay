...
def tiny_llama_loop(filename, dataset, num_llamas=10):
    prompt = open(filename).read().strip()
    llamas = [random.randint(1, 100) for _ in range(num_llamas)]
    modified_records = []
    for record in dataset.records:
        question = record.fields["question"]
        answer = record.fields["answer"]
        to_judge = question + " " + answer
        ratings = list()
        for i in tqdm(range(num_llamas), desc="TinyLlamas are reviewing"):
            answer, rating = tiny_llama_model(prompt, to_judge, llamas[i])
            if rating is not None:
                ratings.append(rating)
                new_content = "[Instruction]" + question + "\n" + "[Answer]" + answer + " - " + str(rating) + "\n\n"
                prompt = new_content + prompt
        record.suggestions = [
            {
                "question_name": "answer_quality",
                "value": int(sum(ratings) / len(ratings)),
                "agent": "TinyLlamas",
            }
        ]
...
if __name__ == "__main__":
    ...
    tiny_llama_loop("sample.txt", dataset_remote, num_llamas=5)
